...
void loop()
{
  ...
  { // Wait for a message
    ...
    {                              // Check if a message is received
      ...
      // Check if the message contains "RP"
      if (strstr((char *)buf, "RP") != NULL)
      {
        Serial.println("Hello World"); // Print "Hello World" to the Serial Monitor
      }

      // Attempt to print out RSSI and SNR (if available)
      ...
      // SNR is not directly available in RH_RF95, but you might be able to approximate or use other indicators
    }
    else
    {
